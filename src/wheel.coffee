cw =
	...
	colorToRGB: (color) ->
		if cw.isRGB(color) then color
		else if cw.isHSL(color) then cw.HSLToRGB(color)
		else if cw.isColorString(color) then cw.stringToRGB(color)

	colorToHSL: (color) ->
		if cw.isHSL(color) then color
		else if cw.isRGB(color) then cw.RGBToHSL(color)
		else if cw.isColorString(color) then cw.stringToHSL(color)

	colorToString: (color) ->
		if cw.isColorString(color) then color
		else if cw.isHSL(color) then cw.HSLToString(color)
		else if cw.isRGB(color) then cw.RGBToString(color)
	...
	stringToRGB: (string) ->
		...
		if string.length == 7
			extract = (part) -> parseInt(string.substring(part*2 + 1, part*2 + 3), 16) / 255
		# use a different extract if form is #abc
		else if string.length == 4
			extract = (part) -> parseInt(string[part+1], 16) / 15
		
		cw.RGB(extract(0), extract(1), extract(2))

	stringToHSL: (string) -> cw.RGBToHSL(cw.stringToRGB(string))
	...
	ColorWheel: (options) ->
		...
		_ping = (enable) ->
			...
			cssBoxShadow = (blur, alpha) ->
				...
				boxShadow: '0 0 ' +blur+ 'px rgba(' +$.map(rgb, to24Bit).join(',')+ ',' +alpha+ ')'
		...
		this._onDocumentDrag = (e) =>
			...
			if _selected == 'ring'
				...
				asPercentage = (n) -> Math.max(0, Math.min(1, n))
